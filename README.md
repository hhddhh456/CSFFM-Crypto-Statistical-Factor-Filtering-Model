# CSFFM - Crypto Statistical Factor Filtering Model 統計因子過濾模型
...
### V3 — Real-Time Runtime Inference Architecture

- Runtime feature recalculation from Binance **1600** 根 1m K 線（`RAW_KLINE_LIMIT`）
- 有效觀察窗 **1500** 分鐘（`FEATURE_WINDOW`）；與 Phase 2 共用 `compute_*` 公式
- Training / inference 分離：parquet 僅供離線訓練；Telegram 推論**不再**讀 `combined_features.parquet` 末列
- Feature hash、stale 檢測與 `align_feature_row` 對齊 RF 欄位順序
- Git 備份標籤：`v2-telegram-parquet-snapshot`（升級前快照）

---

## 1. 功能

* 即時市場追蹤：監控 BTC / ETH 最新價格與 1 分鐘 K 線數據；**推論時在記憶體中即時計算 50 維特徵**（V3），不依賴 Data Lake 末筆 parquet。
#### 量化風險指標預測：
* 24H 未來日化實現波動率（Realized Volatility）預測與方向信心水準。
* 峰度（Kurtosis）極端風險與Tail-Risk事件預測。
* Jarque-Bera 分佈異常檢定。

#### 期權賣方策略動態引擎：
* 基於預期 24H 價格區間，動態計算 Short PUT / Short CALL / Strangle / Iron Condor 推薦履約價。
* 避免四捨五入誤差的 Tick Size 調整（BTC: 500, ETH: 50），嚴防履約價回歸平值（ATM）。
...
系統每日固定觸發 4 次完整報告，完美覆蓋全球加密貨幣與傳統美股市場之關鍵節點：

| 觸發時間 (基準時區) | 時區參考 | 優先級 | 業務場景說明 |
| --- | --- | --- | --- |
| 00:30 UTC | UTC | 中 | 早盤前全球加密市場總覽與基調預測。 |
| 07:50 UTC | UTC | 最高 | Deribit 期權結算前 10 分鐘核心風控報告。 |
...
daily_realized_vol = annualized_vol / sqrt(365)
...
* Fallback: When data staleness or upstream model exceptions are captured, the pipeline switches to a safe fallback payload, guaranteeing zero runtime crashes.
